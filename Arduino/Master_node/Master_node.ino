...
void server_Connection(const char* serverName,float humi,float temp,int gasanalog,int gasdigital) {
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println(humi);
      Serial.print("gas:");
      Serial.println(gasanalog)#include <WiFi.h>
#include <HTTPClient.h>
#include <string.h>
#include "DHT.h" // DHT library from Adafruit
//Adafruit Unified Sensor library.
...
char json[200];
//Your Domain name with URL path or IP address with path
const char* serverName = "http://192.168.45.157:8000/readings";
const char* alertserver="http://192.168.45.157:8000/alert";
...
void server_Connection(const char* serverName,float humi,float temp,int gasanalog,int gasdigital) {
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.print("temperature from master node:");
      Serial.println(temp);
      Serial.print("humidity from master node:");
      Serial.println(humi);
      Serial.print("gas value from master node:");
      Serial.println(gasanalog);
      Serial.print("temperature value from sub node:");
      Serial.println(temperature_value);
      Serial.print("humidity value from sub node:");
      Serial.println(humidity_value);
      Serial.print("gas value from subnode:");
      ...
      // Free resources
      http.end();
      if(temp>25 && gasanalog>=350){
        Serial.print("master node alert send successfully");
        http.begin(client,alertserver);
        http.addHeader("Content-Type", "application/json");
        sprintf(simple_json1, "{\"device_id\":\"%d\",\"temp_reading\":\"%.2f\",\"hum_reading\":\"%.2f\",\"device_status\":\"%d\",\"smoke_reading\":\"%d\"}", 1, temp, humi,device_status,gasanalog);
        int httpResponseCode1 = http.POST(simple_json1);
        delay(2000);
      ...
      if(temperature_value.toInt()>25 && gas_analog.toInt()>=350){
         Serial.print("sub node alert send successfully");
          http.begin(client,alertserver);
          http.addHeader("Content-Type", "application/json");
          sprintf(json, "{\"device_id\":\"%d\",\"temp_reading1\":\"%s\",\"hum_reading1\":\"%s\",\"device_status\":\"%s\",\"gas_analog_reading1\":\"%s\"}", 2,temperature_value,humidity_value,device_status1,gas_analog);
          int httpResponseCode1 = http.POST(json);
          delay(2000);
          http.end();
